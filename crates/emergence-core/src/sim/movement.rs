        ...
        Action::Build => {
            ...
            if world.terrain.structure[cidx] == 0 && !world.terrain.is_water(cx.min(world.terrain.width - 1), cy.min(world.terrain.height - 1)) {
                ...
                if world.terrain.build_progress[cidx] >= build_ticks {
                    ...
                    // Deforestation: clear biomass for construction foundation (also done in place_structure)
                    world.terrain.biomass[cidx] = 0.0;
                    // Degrade soil from construction disturbance
                    world.terrain.nutrient_density[cidx] *= 0.5;
                    // Clear adjacent tiles (1-tile radius) to prevent flora clipping through structure
