...
pub fn compose_from_assets(packs_root: &str) -> (Vec<u8>, Vec<String>) {
    ...
    let blit_cell_1024 = |pixels: &mut [u8], atlas_row: usize, atlas_col: usize, tile: &image::RgbaImage| {
    ...
    for &(npc_num, atlas_row) in npc_map {
        ...
        if let Some(sheet) = load_png(&path) {
            ...
            for atlas_col in 10..16usize {
                let src_row = (atlas_col as u32 - 10 + 4) % 8;
    ...
    for (name, row, col_start) in &[("bird", 12usize, 0usize), ("chicken", 12, 8), ("cow", 13, 0)] {
