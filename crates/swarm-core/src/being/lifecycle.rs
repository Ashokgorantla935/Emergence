...
pub fn check_death_conditions(beings: &mut Beings, season: Season) -> Vec<usize> {
    ...
    for i in 0..beings.count {
        ...
        // Starvation death: 200+ ticks at zero hunger
        if beings.hunger_zero_ticks[i] >= 200 {
            beings.states[i] = BeingState::Dead;
            beings.alive_count -= 1;
            newly_dead.push(i);
            continue;
        ...
        // Exposure death: 100+ ticks at zero warmth in winter
        if season == Season::Winter && beings.warmth_zero_ticks[i] >= 100 {
            beings.states[i] = BeingState::Dead;
            beings.alive_count -= 1;
            newly_dead.push(i);
        }
...
mod tests {
    ...
    fn test_need_decay() {
        ...
        // Hunger: 1.0 - 500 * 0.002 = 0.0
        assert!(
            beings.needs[0][NEED_HUNGER] <= 0.001,
            "hunger should be at 0 after 500 decays, got {}",
